...
<!--        &lt;!&ndash; 1. 左侧：全校健康趋势图 &ndash;&gt;-->
<!--        <section class="main-chart-card">-->
<!--          <h3>📈 Weekly Stress vs Sleep Trends</h3>-->
<!--          <div class="chart-wrapper">-->
<!--            <Line v-if="chartData" :data="chartData" :options="chartOptions" />-->
<!--          </div>-->
<!--        </section>-->

<!--        &lt;!&ndash; 2. 右侧上部：风险预警名单 &ndash;&gt;-->
<!--        <section class="risk-card">-->
<!--          <div class="card-header">-->
<!--            <h3>⚠️ Early Warning List</h3>-->
<!--            <span class="badge">{{ riskList.length }} At Risk</span>-->
<!--          </div>-->
<!--          <div class="risk-list-container">-->
<!--            <div v-if="riskList.length === 0" class="empty-msg">No students at risk.</div>-->
...
<!--                <span v-if="record.stress_level >= 4" class="tag tag-stress">High Stress ({{record.stress_level}})</span>-->
<!--                <span v-if="record.hours_slept < 5" class="tag tag-sleep">Low Sleep ({{record.hours_slept}}h)</span>-->
<!--              </div>-->
<!--            </div>-->
<!--          </div>-->
<!--        </section>-->

<!--        &lt;!&ndash; 3. 右侧下部：快速数据录入 &ndash;&gt;-->
<!--        <section class="entry-card">-->
<!--          <h3>📝 Quick Data Entry</h3>-->
<!--          <form @submit.prevent="submitSurvey" class="entry-form">-->
<!--            <input v-model="form.student_number" placeholder="Student Number (e.g. u123)" required />-->
<!--            <div class="row">-->
<!--              <select v-model.number="form.stress_level" required>-->
<!--                <option value="" disabled>Stress Lvl</option>-->
<!--                <option v-for="n in 5" :key="n" :value="n">{{ n }}</option>-->
<!--              </select>-->
<!--              <input v-model.number="form.hours_slept" type="number" placeholder="Sleep (h)" step="0.5" required />-->
<!--            </div>-->
<!--            <button type="submit" class="btn-save">Save Record</button>-->
<!--            <span v-if="msg" class="success-msg">{{ msg }}</span>-->
<!--          </form>-->
<!--        </section>-->
<!--      </div>-->
<!--    </main>-->
...
<!--import { getWellbeingTrends, getRiskAlerts, createSurvey } from '../api/wellbeing'-->
...
<!--  // 处理图表数据-->
<!--  chartData.value = {-->
<!--    labels: trendsRes.data.map(t => `Week ${t.week}`),-->
<!--    datasets: [-->
<!--      {-->
<!--        label: 'Avg Stress',-->
...
<!--    msg.value = 'Saved!'-->
<!--    await loadDashboard() // 刷新列表-->
<!--    setTimeout(() => msg.value = '', 2000)-->
<!--    form.value.student_number = ''-->
<!--  } catch (e) {-->
<!--    alert('Error saving data. Check student number.')-->
<!--  }-->
<!--}-->
...
<!--<style scoped>-->
<!--/* 复用部分 Navbar 样式 */-->
<!--.navbar { background: #2c7a7b; color: white; padding: 1rem 2rem; display: flex; justify-content: space-between; align-items: center; }-->
<!--.brand { font-size: 1.2rem; font-weight: bold; display: flex; gap: 10px; }-->
<!--.btn-logout { background: #e53e3e; color: white; border: none; padding: 0.5rem 1rem; border-radius: 4px; margin-left: 15px; }-->
...
<!--h3 { margin-top: 0; color: #2d3748; font-size: 1.1rem; border-bottom: 1px solid #edf2f7; padding-bottom: 10px; margin-bottom: 15px; }-->

<!--/* 图表 */-->
<!--.main-chart-card { grid-row: span 2; } /* 左侧占两行高度 */-->
<!--.chart-wrapper { height: 400px; }-->

<!--/* 风险列表 */-->
<!--.risk-card .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }-->
<!--.badge { background: #fed7d7; color: #c53030; padding: 2px 8px; border-radius: 10px; font-size: 0.8rem; font-weight: bold; }-->
<!--.risk-list-container { max-height: 250px; overflow-y: auto; }-->
<!--.student-alert-item { background: #fff5f5; border: 1px solid #feb2b2; padding: 10px; border-radius: 6px; margin-bottom: 8px; }-->
<!--.student-info { display: flex; justify-content: space-between; margin-bottom: 5px; }-->
<!--.tag { font-size: 0.75rem; padding: 2px 6px; border-radius: 4px; margin-right: 5px; color: white; display: inline-block; }-->
<!--.tag-stress { background: #f56565; }-->
<!--.tag-sleep { background: #ecc94b; color: #744210; }-->

<!--/* 录入表单 */-->
<!--.entry-form { display: flex; flex-direction: column; gap: 10px; }-->
<!--.entry-form input, .entry-form select { padding: 8px; border: 1px solid #e2e8f0; border-radius: 4px; }-->
<!--.row { display: flex; gap: 10px; }-->
<!--.row > * { flex: 1; }-->
<!--.btn-save { background: #2c7a7b; color: white; padding: 8px; border: none; border-radius: 4px; cursor: pointer; }-->
<!--.success-msg { color: #38a169; text-align: center; font-size: 0.9rem; font-weight: bold; }-->
<!--</style>-->
<template>
  <div class="dashboard-layout">
    <header class="navbar">
